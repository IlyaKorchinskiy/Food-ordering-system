...
import com.food.ordering.system.order.service.domain.port.input.message.listener.CustomerMessageListener;
...
public class CustomerKafkaListener implements KafkaConsumer<CustomerAvroModel> {
    ...
    public CustomerKafkaListener(
            CustomerMessageListener customerMessageListener, OrderMessagingDataMapper orderMessagingDataMapper) {
    ...
    public void receive(
        ...
        log.info(
                "{} number of customer messages received with keys: {}, partitions: {}, offsets: {}",
                messages.size(),
                keys,
                partitions,
                offsets);
        messages.forEach(customerAvroModel -> {
            CustomerModel customerModel = orderMessagingDataMapper.customerAvroModelToCustomerModel(customerAvroModel);
            customerMessageListener.customerCreated(customerModel);
        });
    }
}
